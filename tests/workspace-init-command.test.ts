...
import { buildImportedWorktreeWorkspaceId, toWorkspaceFolderName } from "@/store/project.utils";
...
describe("branch case and legacy/new folder coexistence", () => {
  ...
  test("two branches differing only in case get distinct worktree folders", async () => {
    ...
    const makeRunner = (calls: Array<{ cwd?: string; command: string }>) =>
      async (args: { cwd?: string; command: string }) => {
        calls.push(args);
        const worktreeMatch = args.command.match(/git worktree add -b "[^"]+" "([^"]+)"/);
        if (worktreeMatch?.[1]) {
    ...
    await useAppStore.getState().createWorkspace({ name: "feature/abc", mode: "branch", fromBranch: "main" });
  ...
  test("creating a workspace with the same branch as a legacy workspace preserves the legacy path entry", async () => {
    ...
    const legacyPath = "/tmp/stave-project/.stave/workspaces/feature__legacy-branch";
    ...
    useAppStore.setState({
      ...
      recentProjects: [{
        ...
        defaultBranch: "main",
        workspaces: [{ id: "ws-legacy", name: "feature/legacy-branch", updatedAt: "2026-04-01T00:00:00.000Z" }],
    ...
    const legacyWorkspace = state.workspaces.find((ws) => ws.id === "ws-legacy");
    ...
    const addCall = runCalls.find((c) => c.command.startsWith("git worktree add"));
  ...
  test("new mixed-case workspace uses unique path; existing lowercase branch uses legacy path unaffected", async () => {
    ...
    useAppStore.setState({
      ...
      recentProjects: [{
        ...
        lastOpenedAt: "2026-04-01T00:00:00.000Z",
        defaultBranch: "main",
        workspaces: [{ id: "ws-existing", name: "feature/existing", updatedAt: "2026-04-01T00:00:00.000Z" }],
    ...
    const newWorkspaceId = useAppStore.getState().workspaces.find((ws) => ws.name === "feature/NewMixedCase")?.id;
    const newPath = newWorkspaceId ? useAppStore.getState().workspacePathById[newWorkspaceId] : undefined;
    ...
    expect(useAppStore.getState().workspacePathById["ws-existing"]).toBe(legacyPath);
