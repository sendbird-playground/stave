...
import { buildCreateWorkspaceBranchPickerRows } from "@/components/layout/CreateWorkspaceBranchPicker.utils";
...
export function CreateWorkspaceBranchPicker({
  ...
  const selectedScope = remoteBranches.includes(value) ? "remote" : "local";
  const highlightedRowIndex = rows.findIndex((row) => row.type === "option" && row.option.value === highlightedValue);
        ...
        {loading && rows.length === 0 ? (
          ...
          <Virtuoso
            ...
            itemContent={(index) => {
              ...
              const isHighlighted = row.option.value === highlightedValue;
