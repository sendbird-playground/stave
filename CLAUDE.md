...
| `the-ipc-schema-sync` | 6-file chain: `electron/providers/types.ts` → `src/lib/providers/provider.types.ts` → `src/types/window-api.d.ts` → `electron/preload.ts` → `electron/main/ipc/schemas.ts` → call sites. Event schemas: `src/lib/providers/schemas.ts`. **Host-service dispatch**: `electron/host-service.ts` — async runtime functions (e.g. `attachSession`) MUST be `await`ed before passing to `respond()`. `JSON.stringify(Promise)` = `{}`. |
